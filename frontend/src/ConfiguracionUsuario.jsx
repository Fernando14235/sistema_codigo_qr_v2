...
function ConfiguracionUsuario({ onRegresar, usuario, token }) {
  return (
    <div className="config-usuario-main">
      ...
      <div className="config-usuario-datos">
        <PushNotificationSettings token={token} usuario={usuario} />
        {usuario?.rol === 'admin' && <NotificationTester token={token} />}
      </div>
    </div>
  );
}
